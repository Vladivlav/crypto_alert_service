# app/services/price_alerts/symbol_manager.rb

class PriceAlerts::SymbolManager
  ...
  def active_symbols
    ...
    if symbols_from_redis.any? && redis_client.ttl(ACTIVE_SYMBOLS_KEY) != -2
      Rails.logger.debug "Using cached symbols from Redis."
      return symbols_from_redis
    end

    Rails.logger.info "Cache expired or empty. Syncing from DB."
