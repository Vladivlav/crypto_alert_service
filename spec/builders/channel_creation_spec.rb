...
module NotificationChannels
  RSpec.describe Builders::ChannelCreation, type: :builder do
    context 'when params contain channel type available for creation' do
      context 'when it is a Telegram channel' do
        it 'returns a contract and a scenario for Telegram channel' do
          ...
          expect(result[:contract]).to eq(Contracts::Telegram)
          expect(result[:scenario]).to eq(Scenarios::CreateTelegramChannel)
        end
      end
    end
