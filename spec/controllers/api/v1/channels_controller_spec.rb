...
RSpec.describe Api::V1::ChannelsController, type: :controller do
  describe "POST create" do
    ...
    before do
      allow(NotificationChannels::Builders::ChannelCreation).to receive(:for).and_return(telegram_factory_config)
    end
    ...
    context "when input is invalid (unsupported channel type)" do
      ...
      before do
        allow(NotificationChannels::Builders::ChannelCreation).to receive(:for).and_raise(
          NotificationChannels::Errors::InvalidChannelType, "Unsupported channel type: sms"
        )
      end
