...
RSpec.describe PriceAlerts::Dispatcher do
  include Dry::Monads[:result]

  let(:redis_client)  { instance_double(Redis) }
  let(:initializer)   { instance_double(PriceAlerts::InitializeDirection) }
  let(:rising_handler) { instance_double(PriceAlerts::HandleRisingPrice) }
  let(:falling_handler) { instance_double(PriceAlerts::HandleFallingPrice) }
